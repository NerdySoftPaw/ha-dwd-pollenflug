 ...
 *
 * No build step, no dependencies: a plain custom element. Drop this file in
 * `config/www/`, register it as a dashboard resource (JavaScript Module), and
 * use `type: custom:dwd-pollenflug-card`.
 *
 * Example:
 *   type: custom:dwd-pollenflug-card
 *   title: Pollenflug – Inseln und Marschen   # optional, auto-derived otherwise
 *   forecast: true                            # show morgen / übermorgen chips
 *   entities:
...
class DwdPollenflugCard extends HTMLElement {
  ...
  _title() {
    ...
    // Try to derive the region from the first entity's device name.
    try {
  ...
  _update() {
    ...
    for (const [entityId, refs] of Object.entries(this._rows)) {
      ...
      refs.name.textContent =
        refs.item.name || attrs.friendly_name || entityId;

      const color = levelColor(today);
      ...
      if (refs.chipEls.length) {
        ...
        refs.chipEls.forEach((chip, i) => {
          ...
          chip.dot.parentElement.parentElement.title = d || "";
        });
      }
    }
...
window.customCards.push({
  ...
  preview: false,
  documentation: "https://github.com/NerdySoftPaw/ha-dwd-pollenflug",
});
